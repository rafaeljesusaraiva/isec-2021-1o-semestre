...
int main( int argc , char *argv[] )
{
	...
	while(1){
		...
		while(nReceivedMessages < MAX_MSGS && toContinue){
			...
			nbytes=recvfrom(s, receivedValue,sizeof(receivedValue), 0, (struct sockaddr *)&cli_addr, &size);
		...
		sprintf_s(response, "Num. mensagens recebidas: %.0f; Total de valores: %.3f",  nReceivedMessages, total);
